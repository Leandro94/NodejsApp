...
//app.use(cookieSession({key: "leandro",secret: process.env["SESSION_SECRET"],cookie: {maxAge: 6000}, {signed: true}}));

//app.use(cookieParser({secret:'leandro'}));
app.use(session({secret: 'leandro'}));
//app.use(session({key: "leandro",secret: process.env["SESSION_SECRET"], cookie: { maxAge: 60000 }}));
app.use(flash());
